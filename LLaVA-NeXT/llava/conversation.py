"""
modified 主要屏蔽了llava conv会自动过滤<image>的代码，现在会保留prompt中原本的<image>
"""
import dataclasses
...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if len(messages) > 0 and type(messages[0][1]) is tuple:
            ...
            elif "<image>" not in init_msg: #modified
                init_msg = init_msg.replace("<image>", "").strip()
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            if self.tokenizer is None:
                self.tokenizer_id="meta-llama/Meta-Llama-3-8B-Instruct",
...
# To avoid loading Meta-Llama-3-8B even when not use it.
