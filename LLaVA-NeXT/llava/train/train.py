...
from llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token, process_randomroi_wrapper
...
enable_ref_bbox=False # move this to args
...
class DataArguments:
    ...
    multi_image_pad_threshold: int = 2 # adjust how many images using pad
    max_box_num: int = 4
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    ###########################################################
    saved_cfg=trainer.model.config
    delattr(saved_cfg,"text_config") #deperacated
    saved_cfg.save_pretrained(output_dir)
    ###########################################################
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    assert (input_ids==IMAGE_TOKEN_INDEX).sum()<=5, str(sources)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        # Modified to load different path data
        if "{" in data_path and "}" in data_path:
            #base_path, file_pattern = re.match(r"^(.*)\{(.*)\}\.json$", data_path).groups()
            file_names = data_path.replace("{","").replace("}","").split(",")
            rank0_print(f"Loading {file_names}")
            data_args.dataset_paths = []
            for file_name in file_names:
                data_args.dataset_paths.append(file_name)
                full_path = file_name
    ...
    def load_masks(self,mask_paths,masks): # loaded masks will be store in masks
        """
            new created for load masks
        """
        image_folder = self.data_args.image_folder
        if isinstance(mask_paths, str):  # 统一为list形式
            mask_paths = [mask_paths]
        
        for idx, p in enumerate(mask_paths):
            mask_p=os.path.join(image_folder, p) # relative path will append image_folder, but abs path will overwrite image_folder
            # for different format of masks
            if mask_p.endswith((".png",".jpg",".PNG",".JPG")):
                _mask=Image.open(mask_p).convert('L')
            elif mask_p.endswith((".npz",".npy")):
                _mask=np.load(mask_p)
                if not isinstance(mask,np.ndarray):
                    _mask = _mask["anomaly_map"]
            else: # there should only be a tag ref_bbox
                _mask=mask_p
            masks[idx] = _mask

    def process_image(self, image_file, overwrite_image_aspect_ratio=None, **kwargs):
        """
            Modified for our training data format and masks, also adapting the image process function
        """
        image_folder = self.data_args.image_folder
        ...
        elif 'randomroi' in image_aspect_ratio: # this part should process an img list instead of an image
            boxes=kwargs.get("boxes",None)
            mask=kwargs.get("mask",None)
            if boxes is not None:
                #boxes_list=[random.shuffle(b) for b in boxes_list]
                image = process_randomroi_wrapper(image_aspect_ratio, image, processor, None, self.data_args.image_grid_pinpoints,boxes, shuffle_box=False, max_box_num=self.data_args.max_box_num) 
            else: # use mask to produce boxes
                image = process_randomroi_wrapper(image_aspect_ratio, image, processor, mask, self.data_args.image_grid_pinpoints, shuffle_box=False, max_box_num=self.data_args.max_box_num) 
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]: 
        ...
        image_num=sources[0]["conversations"][0]["value"].count("<image>")
        
        if "image" in sources[0] or image_num>0:
            if "src" in self.list_data_dict[i]:
                image_file=self.list_data_dict[i]["src"] # we use src instead of image for image paths
            elif "image" in sources[0]:
                 image_file = self.list_data_dict[i]["image"]
            else:
                raise ValueError("no image paths in data, while <image> in prompt")
            ######################### Add mask and bbox ##########################
            if type(image_file) is not list:
                image_file=[image_file]
            if image_num!=len(image_file):
                print("WARNING: found data sample with inconsistent <image> num",image_num,"and images num",len(image_file),"make sure <image> are adding in the data file")
                image_num=len(image_file)
            # load masks
            masks = [None] * image_num
            has_mask = False
            if "mask" in self.list_data_dict[i]:
                has_mask = True
                self.load_masks(self.list_data_dict[i]["mask"],masks)
            if image_num==2 and enable_ref_bbox:
                masks[1]="ref_bbox" # this is a tag to let ref img using the same roi as query img
            # load bbox
            boxes_list = [None] * image_num
            has_box = False
            if 'bbox' in self.list_data_dict[i]:
                has_box = True
                for idx, _box in enumerate(self.list_data_dict[i]['bbox']):
                    boxes_list[idx] = _box
                assert len(boxes_list)==image_num, "every image should have a list of boxes, it can be empty but must have one"
            if (has_box and has_mask):
                # if random.choice([0,1])==0:
                masks = None
                
            if (has_box or has_mask) and len(image_file)<2 :
                image = [self.process_image(img,boxes=boxes,mask=mask) for img,boxes,mask in zip(image_file,boxes_list,masks)]
                image = [[im[0], im[1], "image"] for im in image]
            #######################################################################
            elif type(image_file) is list:
                if image_num < self.data_args.multi_image_pad_threshold:
                    image = [self.process_image(f) for f in image_file]
                    #rank0_print(f"less than {self.data_args.multi_image_pad_threshold} imgs,",image_num,len(image))
                else:
                    image = [self.process_image(f, "pad") for f in image_file]
                    #print("more than 2 imgs,",image_num,len(image))
                # Handling multi images
                # overwrite to process with simple pad x we do need this overwrite
                #if len(image_file) > 1:
                #    image = [self.process_image(f, "pad") for f in image_file]
                image = [[im[0], im[1], "image"] for im in image]
            else:
        ...
        elif "video" in sources[0]: # we do not have video data
        ...
        has_image = (image_num>0) or ("video" in self.list_data_dict[i])
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        ################### I DONT KNOW WHY BUT WE NEED VISION CONFIG #####################
        #vision_tower_cfg = AutoConfig.from_pretrained(cfg_pretrained.mm_vision_tower)
        #setattr(cfg_pretrained,"vision_config",vision_tower_cfg.vision_config)
        #setattr(cfg_pretrained,"model_type","llava_onevision")
        #cfg_pretrained.save_pretrained("./config_debug")
        #input()
        ###################################################################################
...
def train(attn_implementation=None):
    ...
    print("Tokenizer 1:", model.config.vocab_size)
    ...
    print("Tokenizer 2:", model.config.vocab_size)
    ...
    print("After trained built:", model.config.vocab_size)
