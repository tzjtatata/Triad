...
# add non use keyword device for compatility
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    # modified pop out non used keyword
    if "device" in kwargs:
        kwargs.pop("device")
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if not vision_tower.is_loaded:
            # Check if the vision tower is loaded after model.
            print("WARNING: Load vision tower after model loaded.")
